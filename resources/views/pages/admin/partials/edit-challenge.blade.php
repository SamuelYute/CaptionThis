@component('components.modal', ['id' => 'edit-challenge-modal-'.$data->id, 'title' => 'Edit Challenge' , 'icon' => 'fa-trophy'])
{!! Form::open(['route' => ['admin.challenges.update', $data->id], 'method' => 'PUT' , 'files' => 'true'])!!}
{{ csrf_field() }}

<div class="modal-body" style="text-align:left">
    <div class="form-group w3-margin {{ $errors->has('title') ? ' has-danger' : '' }}">
        <label for="title">Title</label>
        <input type="text" name="title" class="form-control {{ $errors->has('title') ? ' form-control-danger' : '' }}" id="title" value="{{ old('title',$data->title) }}">
        @if ($errors->has('title'))
            <div class="clear-fix">
            </div>
    ...
    <div class="form-group w3-margin {{ $errors->has('description') ? ' has-danger' : '' }}">
        ...
        <textarea id="description" name="description" rows="3" class="form-control {{ $errors->has('description') ? ' form-control-danger' : '' }}">{{ old('description',$data->description)}}</textarea>
    ...
    <div class="form-group w3-margin {{ $errors->has('start_date') ? ' has-danger' : '' }}">
        ...
        <input id="start_date" type="date" name="start_date" class="form-control {{ $errors->has('start_date') ? ' form-control-danger' : '' }}" value="{{ old('start_date',$data->start_date) }}">
    ...
    <div class="form-group w3-margin {{ $errors->has('end_date') ? ' has-danger' : '' }}">
        ...
        <input id="end_date" type="date" name="end_date" class="form-control {{ $errors->has('end_date') ? ' form-control-danger' : '' }}" value="{{ old('end_date',$data->end_date) }}">
    ...
    <div class="form-group w3-margin {{ $errors->has('sponsor') ? ' has-danger' : '' }}">
        ...
        <input id="sponsor" type="text" name="sponsor" class="form-control {{ $errors->has('sponsor') ? ' form-control-danger' : '' }}" value="{{ old('sponsor',$data->sponsor) }}">
