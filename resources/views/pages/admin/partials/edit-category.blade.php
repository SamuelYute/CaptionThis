@component('components.modal', ['id' => 'edit-category-modal-'.$data->id, 'title' => 'Edit Category' , 'icon' => 'fa-bookmark'])
{!! Form::open(['route' => ['admin.categories.update',$data->id], 'method' => 'PUT' , 'files' => 'true'])!!}
{{ csrf_field() }}

<div class="modal-body" style="text-align:left">
    <div class="form-group w3-margin {{ $errors->has('name') ? ' has-danger' : '' }}">
        <label for="name">Name</label>
        <input type="text" name="name" class="form-control {{ $errors->has('name') ? ' form-control-danger' : '' }}" id="name" value="{{ old('name',$category->name) }}">
        @if ($errors->has('name'))
            <div class="clear-fix">
            </div>
...
<div class="modal-footer">
    <button class="btn w3-green w3-hover-shadow" type="submit"><i class="fa fa-save"></i> Update</button>
